...
async function fetchCart() {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/cart?email=${encodeURIComponent(userEmail)}`);
        const data = await response.json();
        if (data.success) {
...
async function getUserAddresses() {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/addresses?email=${encodeURIComponent(userEmail)}`);
        const data = await response.json();
        if (data.success) {
...
async function placeOrder() {
    ...
    const addresses = await getUserAddresses();
    const selectedAddress = addresses.find(addr => addr.id === parseInt(selectedAddressRadio.value));
...
function setupCheckout() {
    ...
    if (saveAddressBtn) {
        saveAddressBtn.addEventListener('click', async () => {
            ...
            const success = await saveAddressDb({ name, phone, street, barangay, city, province, postal });
